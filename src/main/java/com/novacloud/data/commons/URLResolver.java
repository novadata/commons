...
/**
 * The UrlResolve handles the process of resolving a relative URL against a base
 * URL (context).
 */
public final class URLResolver {
	...
	private static Url parseUrl(final String spec) {
		...
		// URL <path> and the
		// slash "/" that may precede it. Even though the initial slash is not
		// part of the URL path,
		// the parser must remember whether or not it was present so that later
		// processes can
		// differentiate between relative and absolute paths. Often this is done
		// by simply storing
	...
	private static Url resolveUrl(final Url baseUrl, final String relativeUrl) {
		...
		// b) If the embedded URL starts with a scheme name, it is interpreted
		// as an absolute URL
		// and we are done.
		if (url.scheme_ != null) {
			return url;
		...
		// Step 3: If the embedded URL's <net_loc> is non-empty, we skip to Step
		// 7. Otherwise, the
		...
		// a) All occurrences of "./", where "." is a complete path segment, are
		// removed.
		...
		// b) If the path ends with "." as a complete path segment, that "." is
		// removed.
		if (path.endsWith("/.")) {
			path = path.substring(0, path.length() - 1);
		}

		// c) All occurrences of "<segment>/../", where <segment> is a complete
		// path segment not
		// equal to "..", are removed. Removal of these path segments is
	...
	private static class Url {
		...
		/**
		 * Creates an <tt>Url</tt> object from the specified <tt>Url</tt>
		 * object.
		 * 
		 * @param url
		 *            an <tt>Url</tt> object.
