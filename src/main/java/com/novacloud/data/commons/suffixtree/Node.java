 ...
 * 
 * <p>
 * <b>Note:</b> This class is used only internally by
 * {@link GeneralizedSuffixTree}.
 * </p>
 * 
...
class Node {
	...
	/**
	 * The payload array used to store the data (indexes) associated with this
	 * node. In this case, it is used to store all property indexes.
	 * 
	 * As it is handled, it resembles an ArrayList: when it becomes full it is
	 * copied to another bigger array (whose size is equals to data.length +
	 * INCREMENT).
	 * 
	 ...
	 */
	private final CharHashMap<Edge> edges;

	/**
	 * The suffix link as described in Ukkonen's paper. if str is the string
	 * denoted by the path from the root to this, this.suffix is the node
	 * denoted by the path that corresponds to str without the first char.
	 */
	private Node suffix;
	...
	/**
	 * Returns the first <tt>numElements</tt> elements from the ones associated
	 * to this node.
	 * 
	 * Gets data from the payload of both this node and its children, the string
	 * representation of the path to this node is a substring of the one of the
	 * children nodes.
	 * 
	 * @param numElements
	 *            the number of results to return. Use -1 to get all
	 * @return the first <tt>numElements</tt> associated to this node and
	 *         children
	 */
	Collection<Integer> getData(int numElements) {
		...
		// need to get more matches from child nodes. This is what may waste
		// time
		for (Edge e : edges.values()) {
			if (-1 == numElements || ret.size() < numElements) {
				for (int num : e.getDest().getData()) {
					// was getData(missingItems), but this is wrong imho as it
					// would prevent getting certain values,
					// e.g. this node contained indexes 1 & 2, and the
					// numElements is set to 3
					// when traversing to child nodes, we would only receive 1
					// result, but when a child node
	...
	/**
	 * Adds the given <tt>index</tt> to the set of indexes associated with
	 * <tt>this</tt>.
	 * 
	 * @param index
