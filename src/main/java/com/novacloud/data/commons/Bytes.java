...
public class Bytes {
	...
	interface Comparer<T> {
		int compareTo(T buffer1, int offset1, int length1, T buffer2,
	...
	static class LexicographicalComparerHolder {
		...
		public static boolean equals(final byte[] left, int leftOffset,
				int leftLen, final byte[] right, int rightOffset, int rightLen) {
			// short circuit case
			if (left == right && leftOffset == rightOffset
					&& leftLen == rightLen) {
				return true;
			}
		...
		public static boolean equals(byte[] a, ByteBuffer buf) {
			...
			// Thou shalt not modify the original byte buffer in what should be
			// read
			// only operations.
		...
		public static boolean startsWith(byte[] bytes, byte[] prefix) {
			return bytes != null
					...
					&& LexicographicalComparerHolder.BEST_COMPARER.compareTo(
							bytes, 0, prefix.length, prefix, 0, prefix.length) == 0;
		}
		 ...
		 * @param length
		 *            new array size
		 * @return Value in <code>a</code> plus <code>length</code> appended 0
		 *         bytes
		 */
		 ...
		 * math. Useful splitting ranges for MapReduce jobs.
		 * 
		 * @param a
		 *            Beginning of range
		 * @param b
		 *            End of range
		 * @param num
		 *            Number of times to split range. Pass 1 if you want to
		 *            split the range in two; i.e. one split.
		 ...
		 *            generally exclusive and manual splits with an explicit
		 ...
		 */
		public static Iterable<byte[]> iterateOnSplits(final byte[] a,
				final byte[] b, boolean inclusive, final int num) {
			byte[] aPadded;
			byte[] bPadded;
			...
			final BigInteger startBI = new BigInteger(add(prependHeader,
					aPadded));
			final BigInteger stopBI = new BigInteger(
					add(prependHeader, bPadded));
			...
			final Iterator<byte[]> iterator = new Iterator<byte[]>() {
				...
				public byte[] next() {
					...
					BigInteger curBI = startBI.add(intervalBI
		...
		public static byte[] incrementBytes(byte[] value, long amount) {
			...
			if (val.length < SIZEOF_LONG) {
				...
				System.arraycopy(val, 0, newvalue,
						newvalue.length - val.length, val.length);
				val = newvalue;
		...
		public static void writeStringFixedSize(final DataOutput out, String s,
			...
			byte[] b = toBytes(s);
			if (b.length > size) {
				throw new IOException("Trying to write " + b.length
		...
		/**
		 * Copy the byte array given in parameter and return an instance of a
		 * new byte array with the same length and the same content.
		 * 
		 * @param bytes
		 ...
		 * @param length
		 */
		public static byte[] copy(byte[] bytes, final int offset,
				final int length) {
			if (bytes == null)
				return null;
			byte[] result = new byte[length];
		...
		/**
		 * Search sorted array "a" for byte "key". I can't remember if I wrote
		 * this or copied it from somewhere. (mcorgan)
		 * 
		 * @param a
		 ...
		 *         where negative indicates "not found" and the "index + 1"
		 *         handles the "-0" case.
		 */
		...
		/**
		 * Treat the byte[] as an unsigned series of bytes, most significant
		 * bits first. Start by adding 1 to the rightmost bit/byte and carry
		 * over all overflows to the more significant bits/bytes.
		 * 
		 * @param input
		...
		/**
		 * Returns the index of the first appearance of the value {@code target}
		 * in {@code array}.
		 * 
		 * @param array
		 ...
		 * @return the least index {@code i} for which
		 ...
		 * {@code target} within {@code array}, or {@code -1} if there is no
		 * such occurrence.
		 * 
		 * <p>
		 ...
		 * @param target
		 *            a primitive {@code byte} value
		 * @return {@code true} if {@code target} is present as an element
		 *         anywhere in {@code array}.
		 */
